...
<a href="<?php echo APP_PATH;?>index.php?m=music&c=admin_music&a=addpos&id=<?php echo $id.'&search='.$_GET['search']?>">添加试听歌曲</a>
</div>
<div class="bk10"></div>
...
?>
    <tr>
    	...
    	<td>
    		<a name='delete' style='cursor:pointer;'onclick='javascript:deleteops(<?php echo $v["id"]?>)'><?php echo L('delete')?></a>
    	</td>	
...
<div class="btn">
	...
	<input type="button" class="button" id='deletall'name="dosubmit" value="删除所有"/>
</div> 
...
// 移除试听歌曲
function deleteops(ids)
		{	

			if(confirm("确定移除吗？"))
			{
				$.post(
					'?m=music&c=admin_music&a=deletepos',
					{id:sid,ids:ids},
					function(statu)
					{
