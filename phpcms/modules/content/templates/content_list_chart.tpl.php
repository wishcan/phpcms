...
<tbody>
	...
	?>
		...
		<td align='center'>
			...
			<a href='<?php echo APP_PATH.'index.php?m=charts&c=charts&a=admin&id='.$r['id']?>' target='right'>查看榜单</a>
			<a href='javascript:change(<?php echo $r["id"];?>,<?php echo $r["statu"];?>)'>
			<?php switch (intval($r['statu'])) {
				case 1:
				echo "<span style='color:#f01bca'>取消发布</span>";
				break;
				case 2:
				echo "<span style='color:#aeb409'>发布榜单</span>";
				break;
				}?>
...
// 选择歌曲添加进入帮当
function view_music(id, name,catid,selfid) {
	window.top.art.dialog({id:'view_comment'}).close();
	 charid=catid;
	 catid=catid-20;
	
	window.top.art.dialog({yesText:'<?php echo L('dialog_close');?>',title:'添加打榜歌曲：'+name,id:'view_comment',iframe:'index.php?m=content&c=content&a=init&menuid=822&catid='+catid+'&charid='+charid+'&selfid='+selfid+'&pc_hash=GEc1ri',width:'900',height:'500'}, function(){window.top.art.dialog({id:'edit'}).close()});
}
...
{
	$.post(
		"<?php echo WEB_PATH;?>index.php?m=charts&c=charts&a=changeType",
		{id:id,statu:statu},
		function(statu){
