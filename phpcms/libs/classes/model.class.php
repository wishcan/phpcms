...
class model {
	...
	final public function listinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array()) {
		...
		/**
		 * 修改了分页样式
		 * @param $where['count'] 统计的方法
		 * @param $where['sql']   为去掉select后的语句
		 * @param $
		 */
	final public function my_listinfo($where = '', $order = '', $page = 1, $pagesize = 20, $key='', $setpages = 10,$urlrule = '',$array = array()) {
		if(is_array($where)){
	 ...
	 * @return [array]  $row 获得所有的查询
	 */
	final public function queryAll($sql)
	{
